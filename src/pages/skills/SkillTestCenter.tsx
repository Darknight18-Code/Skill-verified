...
import { getSkillCategoriesWithDurations, SkillCategory } from '../../data/skillCategories';
...
export const SkillTestCenter = () => {
  ...
  const stats = [
    { label: 'Available Tests', value: categories.reduce((sum, cat) => sum + (cat.tests?.length || 0), 0).toString(), icon: BookOpen, color: 'bg-blue-500' },
  ...
  React.useEffect(() => {
    setLoading(true);
    getSkillCategoriesWithDurations()
